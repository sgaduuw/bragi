...
Two exercises: (1) the Jinja global is registered on a delivery
app, (2) the shipped partial renders correctly when included
into a minimal page.
"""
...
@pytest.fixture
def app(patched_session_locals, db_session) -> Flask:
    site = make_test_site(
        db_session,
        slug="t",
        hostname="t.example",
    ...
    db_session.add_all([
        Page(
            ...
            kind=PageKind.STATIC, menu_order=10, show_in_nav=True,
        ),
        Page(
            site_id=site.id, slug="projects", title="Projects",
            author_id=user.id, status=PageStatus.PUBLISHED,
            kind=PageKind.STATIC, menu_order=1, show_in_nav=True,
        ),
        Page(
            site_id=site.id, slug="hidden", title="Hidden",
            author_id=user.id, status=PageStatus.PUBLISHED,
            kind=PageKind.STATIC, menu_order=0, show_in_nav=False,
        ),
    ])
    db_session.commit()
...
def test_partial_renders_expected_items_in_order(app: Flask, db_session) -> None:
    # Re-fetch the site so we have it in scope for g.site.
    site = db_session.execute(
        select(Site).where(Site.hostname == "t.example")
    ).scalar_one()
...
def test_partial_renders_nothing_when_tree_is_empty(
    patched_session_locals, db_session,
) -> None:
